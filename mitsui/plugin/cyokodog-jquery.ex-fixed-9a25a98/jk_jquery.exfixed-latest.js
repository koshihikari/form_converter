...
(function($jk){
	...
	var API = function(api){
		var api = $jk(api),api0 = api[0];
		for(var name in api0)
			(function(name){
				if($jk.isFunction( api0[name] ))
	...
	var scrollEvent = function( target , config ){
		...
		var c = o.config = $jk.extend({},scrollEvent.defaults,config,{
			target : target
		});
		c.status = 0;
		c.scroll = o.getPos();
	...
	$jk.extend(scrollEvent.prototype,{
		isMove : function(){
	...
	var prop = function(){
		return $jk.fn.prop ? 'prop' : 'attr';
	}

	$jk.ex = $jk.ex || {};

	$jk.ex.fixed = function(idx , targets , option){
		var o = this,
		c = o.config = $jk.extend({} , $jk.ex.fixed.defaults , option);
		c.targets = targets;
		...
		c = $jk.extend(c,{
			...
			style : '',
			window : $jk(window),
			oldBrowser : $jk.browser.msie && ($jk.browser.version < 7.0 || !$jk.boxModel)
		});
		c.dynamicMode = c.baseNode || !c.fixedX || !c.fixedY;

		if (c.dynamicFixed || c.dynamicMode || c.oldBrowser){
			$jk('body').append(c.target);
		}

		if (c.baseNode) c.baseNode = $jk(c.baseNode);
...
//		c.container = $jk.boxModel && !$jk.browser.opera ? $jk('html') : $jk('body');
		c.container = $jk.boxModel ? $jk('html') : $jk('body');
...
//		if (c.oldBrowser && !/hidden|scroll/i.test(c.target.css('overflow'))) {
	...
	$jk.extend($jk.ex.fixed.prototype, {
		_attn :[
		...
		_smoothPatch : function(){
			...
			$jk.ex.fixed.config.smoothPatched = true;
...
//					var over = (pos1 + c.target['outer'+cm.size]()) - c.container.attr('client'+cm.size);
	...
	$jk.fn.exFixed = function(option){
		var targets = this,api = [];
		targets.each(function(idx) {
			var target = targets.eq(idx);
			var obj = target.data('ex-fixed') || new $jk.ex.fixed( idx , targets , option);
			api.push(obj);
